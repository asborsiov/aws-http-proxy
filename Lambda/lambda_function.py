...
def run_fargate_task(subnet):
  ...
  overrides = {
    'containerOverrides': [
     {
     'name': "Dante",
     'environment': [{'name': "PROXY_LIFETIME", "value": "90"}]
      }
      ]
  })
...
def get_public_ip(fargate_task_arn):
  ...
  l = task['tasks'][0]['attachments'][0]['details']
  eni_id = ""
  for t in range(len(l)):
    if(task['tasks'][0]['attachments'][0]['details'][t]['name'] != "networkInterfaceId"): 
      continue
    eni_id = task['tasks'][0]['attachments'][0]['details'][t]['value']
    break;
    
  eni = boto3.resource('ec2', region_name=region).NetworkInterface(eni_id)
